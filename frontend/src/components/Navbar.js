    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 lg:px-8 h-20">
        ...
        <a href="#hero" className="flex items-center gap-3 no-underline group">
          <div className="w-10 h-10 border border-gold/30 flex items-center justify-center group-hover:border-gold/60 group-hover:shadow-[0_0_15px_rgba(201,168,76,0.15)] transition-all duration-500">
            <span className="text-gold text-lg group-hover:scale-110 transition-transform duration-300">
              \u0950
            </span>
          </div>
          <div className="flex flex-col">
            <span className="font-heading text-[12px] tracking-[0.15em] uppercase text-gold leading-tight">
              Arun Valley
            </span>
            <span className="text-[11px] text-cream-muted tracking-wide">
              Rudraksha
            </span>
          </div>
        </a>
        ...
        <div className="flex items-center gap-4">
          <a
            ...
            className="hidden sm:inline-flex items-center gap-2 bg-green text-cream px-5 py-2.5 text-[12px] font-heading tracking-[0.1em] uppercase border border-green-light hover:bg-green-light hover:shadow-[0_0_20px_rgba(42,90,68,0.3)] transition-all duration-300 btn-shine"
          >
      ...
      {/* Mobile Menu */}
      <div
        className={`lg:hidden fixed inset-0 top-20 bg-[rgba(13,10,6,0.98)] backdrop-blur-2xl transition-all duration-500 ${
          mobileOpen
            ? "opacity-100 pointer-events-auto"
            : "opacity-0 pointer-events-none"
        }`}
      >
        <div className="px-8 pt-8 flex flex-col gap-0">
          {navLinks.map((link, i) => (
            <a
              key={link.href}
              href={link.href}
              onClick={() => setMobileOpen(false)}
              className={`block py-4 text-cream-muted hover:text-gold border-b border-cream-border text-[18px] tracking-wide transition-all duration-500 ${
                mobileOpen
